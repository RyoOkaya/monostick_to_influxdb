...
def sendHTTPRequest(l):
    ...
    tag_keys = 'logical_device_id=' + str(l[0]) + ',packet_id=' + str(l[2]) + ',physical_device_id=' + str(l[5] << 24 | l[6] << 16 | l[7] << 8 | l[8]) # 索引付き列
    field_keys = 'LQI=' + str(l[4]) + ',mV=' + str(l[13] << 8 | l[14]) + ',open=' + str(0 if (l[16] & 0x1) == 0 else 1) + ',relay_count=' + str(l[12]) # 索引なし列
...
# 受信データの表示 (IO状態の通知はコマンド用データ値が必ず0x81)
...
def readThread():
    ...
    while True:
        ...
        line = ser.readline().rstrip() # 1行単位で読み出し(データ受信まで待機)、末尾の改行コードを削除
        
        if len(line) > 0 and chr(line[0]) == ':':
            print (datetime.now().strftime("%Y/%m/%d %H:%M:%S"))
            print ("  row binary data                   : %s" % line)
        else : # 想定外のデータ受信は無視
            continue
        ...
        try:
            ...
            if csum == 0:
                if lst[1] == 0x81: # IO関連のデータの受信
                    printPayload_0x81(lst) # 受信データを解析して表示
                    sendHTTPRequest(lst) # HTTP送信
                else: # その他のデータ受信
                    printPayload(lst) # 受信データをそのまま表示
            else:
                print ("checksum ng")
        except:
            if len(line) > 0: # エラー時
